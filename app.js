...
function exportarExcel() {
    ...
    XLSX.writeFile(wb, "Reporte_AHCJ_Final.xlsx");
}
    ...
    try {
        ...
        canvas.toBlob(async (blob) => {
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({ files: [file], title: 'Recibo AHCJ', text: `Recibo de ${r.Jugador}` });
            } else {
                window.open(`https://wa.me/?text=*RECIBO AHCJ*%0ASocio: ${r.Jugador}%0AFolio: ${r.Nro_Folio}`, '_blank');
            }
        });
